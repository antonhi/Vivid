...
class NavigationBarComponent extends StatelessWidget {
  ...
  static Map<Icon, Function()> getMainOptions(int page, BuildContext context) {
    ...
    options[Icon(Icons.play_arrow_rounded, color: page == 1 ? AppColorConfiguration.white :
      AppColorConfiguration.secondary, size: 30,)] = () => BlocProvider.of<AppController>(context).showFeed();
    options[Icon(Icons.person, color: page == 2 ? AppColorConfiguration.white :
    AppColorConfiguration.secondary, size: 30)] = () => BlocProvider.of<AppController>(context).showAuthentication();
    options[Icon(Icons.mail_outline_rounded, color: page == 3 ? AppColorConfiguration.white :
    AppColorConfiguration.secondary, size: 30)] = () => BlocProvider.of<AppController>(context).showFeed();
    return options;
  }
